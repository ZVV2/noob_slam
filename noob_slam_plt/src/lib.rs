...
pub fn quick_plot_single<const C : usize>(map : &OccupMap<C>, path : &str, settings : PlotSettings) -> Result<(), Box<dyn std::error::Error>> {
...
pub fn quick_plot_dual<const C : usize>(ref_map : &OccupMap<C>, input_map : &OccupMap<C>, offset_x : usize, offset_y : usize, path : &str, settings : PlotSettings) -> Result<(), Box<dyn std::error::Error>> {
    // Number of rows and columns in the grid
