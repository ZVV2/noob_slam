...
use noob_slam_plt::{PlotSettings, quick_plot_single};
...
fn sample_down() {
    ...
    quick_plot_single(&map, "data/1_sample_down_f1.png", PlotSettings::default()).unwrap();
    ...
    for factor in 2 ..=10 {
        ...
        quick_plot_single(&new_map, format!("data/1_sample_down_f{}.png", factor).as_str(), PlotSettings::default()).unwrap();
    }
}
...
fn correlation_tile_grid_vs_sample_down() {
    ...
    // Tests
    println!("> [TEST] Correlation comparison - Downsampling vs. tile-grid");
